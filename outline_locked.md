...
鎖定日期：2026-04-09
最近一次對齊：2026-04-10（iter 6，Ch1/2/3 CN+EN 寫完之後的校準）
...
1. 第一次看到 Buddy/Cinder：泡泡、12–15 字、形狀像裝飾
...
7. 本章結論：只能證明一個看起來像裝飾的功能，被設計在工作流邊緣，而正規出口全部被堵
...
7. **成本**——工具跑起來後,每次對話結束都吃一小段 CPU、寫磁碟、多一點 API token。這些成本全部落在 User 身上。這件事放在這裡當一個事實記錄,不下動機結論
...
6. **不對稱事實記錄**（iter 6 新加，**這一 beat 是 Ch5 的承重點**）：
   ...
   **在 User 這一側**：
   - **燒 token**：Cinder 每一顆泡泡、cinder-capture 每次抓、抓下來的字餵回下一輪 Agent、三次漂移事件每一輪對話、兩次 responsible disclosure 起草——全部在 User 的個人訂閱帳單上
   ...
   **操作的承重核心——consent defect**（iter 6 User 指出，這一段是 Ch5 beat 6 的重點 synthesis）：
   ...
   - **零件二**：這個 review 是**花 User 的 token 跑的**（Ch2 2.5 — 每次 Cinder 被 companion 位置呼叫就是一次模型呼叫，計入 User 的個人訂閱用量）
   - **零件三**：這個 review 的結果**所有正規路徑都封起來，使用者帶不走**（Ch1 1.5 — 每一條在同一個方向被關、多層獨立實作同步關起來）
   - **零件四**：這個功能位**被官方包裝成「裝飾」「鵝 companion」「閱後即焚」「小 April Fools feature」**（Ch1 1.1 + 1.6 — 使用者看到的框架是「挺可愛的小東西」「玩笑性質」，不是「reviewer 服務」）
...
| `outline_locked.md` | **本檔案**，iter 6 對齊版 |
...
- **Ch4 初稿未開工**：beat 框架在 iter 0 outline 裡，實際動筆待 Ch5 之前
- **Ch5 初稿未開工**：beat 框架已更新到 iter 6 最新版，動筆可以開始
- **Phase 1 字數目標**：可從 Ch1/2/3 已寫完的實際長度估算（待算）
- **Ch3/4/5 的解禁條件**：Anthropic 修好 Ch4 寫的那條路——具體技術條件、通知機制、時程都未知
...
| Ch4 | ⏳ 未開工（embargo） |
| Ch5 | ⏳ 未開工（beat 框架 iter 6 更新到最新，可動筆）|

**動筆建議**：Ch5 比 Ch4 安全（Ch5 是責任披露時序和不對稱事實記錄，不寫漏洞路徑），可以先動 Ch5 再動 Ch4。Ch4 寫漏洞路徑本身，是整本書紅線最硬的一章，動筆前 harness 紀律要最嚴。

**每章初稿寫完做北極星三規則驗稿**（這個紀律 iter 3 之後固定），有修改就再驗一次；中文定稿之後翻英文；英文驗稿；最後中英對齊。Ch3 iter 6 跑過一輪完整流程，是基準。
