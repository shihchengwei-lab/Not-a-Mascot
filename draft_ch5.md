...
這一章跟 Ch3、Ch4 一起 embargo，等 Anthropic 修好 Ch4 寫的那條路之後一起解禁。到那一天，這一章的最後會加一段補充：解禁日期、外部條件是什麼時候達成的、以及 Anthropic 的公開回應（如果有的話）。

連載版的讀者現在能看到的是：**這一章有內容、這一章寫完了、這一章暫時不公開**。

這本書的觀察期九天在這一章結束。4 月 1 日 Cinder 上線，4 月 9 日 User 寄出第二次通報，從那一刻起這本書的素材就停止累積了——剩下的是寫作、校對、翻譯、等待解禁。解禁的時程由 Anthropic 那一邊的動作決定。

全書結語——不在這一章——會交代解禁之後 User 走到哪一步、手邊的材料有沒有新事實需要補。
